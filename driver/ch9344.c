/*
 * USB serial driver for USB to Quad UARTs chip ch9344.
 *
 ...
 * V1.17 - added supports for break function
 */
...
#define DRIVER_DESC "USB serial driver for ch9344."
#define VERSION_DESC "V1.17 On 2021.06.30"
...
static int ch9344_set_uartmode(struct ch9344 *ch9344, int portnum, u8 index,
                               u8 mode);
...
static int ch9344_get_portnum(int index) { return index % NUMSTEP; }

/*
 * Functions for CH9344 control messages.
 */
static int ch9344_control_in(struct ch9344 *ch9344, u8 request, u16 value,
                             u16 index, char *buf, unsigned bufsize) {
  int retval;

  retval =
      usb_control_msg(ch9344->dev, usb_rcvctrlpipe(ch9344->dev, 0), request,
                      USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_DIR_IN, value,
                      index, buf, bufsize, DEFAULT_TIMEOUT);

  dev_vdbg(&ch9344->data->dev, "ch9344_control_in(%02x,%02x,%04x,%04x,%p,%u)\n",
...
static int ch9344_cmd_out(struct ch9344 *ch9344, u8 *buf, int count) {
  ...
  retval = usb_bulk_msg(ch9344->dev, ch9344->cmdtx_endpoint, buf,
                        min((unsigned int)count, ch9344->cmdsize), &actual_len,
                        DEFAULT_TIMEOUT);

  if (retval) {
    dev_dbg(&ch9344->data->dev, "usb_bulk_msg(send) failed, err %i\n", retval);
    return retval;
  }
      ...
      if (reg_iir >= R_MOD && reg_iir <= R_IO_I) {
        ...
        if ((portnum >= PORT_OFFSET) || (portnum < 2 * PORT_OFFSET)) {
          ...
          if (!ch9344->clocal[portnum] &&
              (ch9344->ctrlin[portnum] & ~newctrl & CH9344_CTI_DC)) {
...
static int ch9344_tty_open(struct tty_struct *tty, struct file *filp) {
  ...
  return tty_port_open(&ch9344->ttyport[ch9344_get_portnum(tty->index)].port,
                       tty, filp);
}
...
static void ch9344_tty_close(struct tty_struct *tty, struct file *filp) {
  ...
  if (ch9344->uartmode[portnum] == M_HF) {
    ret = ch9344_set_uartmode(ch9344, portnum, portnum, M_NOR);
    if (!ret) {
      ch9344->uartmode[portnum] = M_NOR;
    }
  }
}

static int ch9344_tty_write(struct tty_struct *tty, const unsigned char *buf,
                            int count) {
  struct ch9344 *ch9344 = tty->driver_data;
  int stat;
  unsigned long flags;
...
static int ch9344_tty_tiocmget(struct tty_struct *tty) {
  ...
  result = (ch9344->ctrlout[portnum] & CH9344_CTO_D ? TIOCM_DTR : 0) |
           (ch9344->ctrlout[portnum] & CH9344_CTO_R ? TIOCM_RTS : 0) |
           (ch9344->ctrlin[portnum] & CH9344_CTI_C ? TIOCM_CTS : 0) |
           (ch9344->ctrlin[portnum] & CH9344_CTI_DS ? TIOCM_DSR : 0) |
           (ch9344->ctrlin[portnum] & CH9344_CTI_R ? TIOCM_RI : 0) |
           (ch9344->ctrlin[portnum] & CH9344_CTI_DC ? TIOCM_CD : 0);
...
static int ch9344_set_uartmode(struct ch9344 *ch9344, int portnum, u8 index,
  ...
  if ((ch9344->uartmode[portnum] == M_NOR) && (mode == M_HF)) {
    rgadd = 0x10 * portnum + 0x08;
    buffer[0] = CMD_W_BR;
    buffer[1] = rgadd + R_C4;
    buffer[2] = 0x51;
  ...
  if ((ch9344->uartmode[portnum] == M_HF) && (mode == M_NOR)) {
    buffer[0] = CMD_W_BR;
    buffer[1] = rgadd + R_C4;
    buffer[2] = 0x50;
    ret = ch9344_cmd_out(ch9344, buffer, 0x03);
  ...
  mode4 |= mode << (index * 2);
  request = CMD_WB_E + portnum + PORT_OFFSET;
  rgadd = R_MOD;
...
static int ch9344_set_gpiodir(struct ch9344 *ch9344, int portnum, u8 gpionumber,
                              u8 gpiodir) {
  u8 *buffer;
  int ret;
  u8 request;
...
static int ch9344_set_gpioval(struct ch9344 *ch9344, int portnum, u8 gpionumber,
                              u8 gpioval) {
  u8 *buffer;
  int ret;
  u8 request;
...
static int ch9344_tty_ioctl(struct tty_struct *tty, unsigned int cmd,
  ...
  case IOCTL_CMD_GPIOENABLE:
    ...
    if (inargL)
      ...
      uartmode = M_NOR;
    if ((portindex > 3) || (ch9344->uartmode[portindex] == M_HF)) {
      rv = -EINVAL;
      goto out;
    }
  ...
  case IOCTL_CMD_GPIODIR:
    ...
    if (inargL)
      ...
      gpiodir = G_DI;
    if ((gpionumber > MAXGPIO) || (ch9344->uartmode[gpiogroup] != M_IO)) {
      rv = -EINVAL;
      goto out;
    }
    mutex_lock(&ch9344->gpiomutex);
...
static void ch9344_tty_set_termios(struct tty_struct *tty,
  ...
  if (memcmp(&ch9344->line[portnum], &newline, sizeof newline)) {
    ...
    dev_vdbg(&ch9344->data->dev, "%s - set line: %d %d %d %d\n", __func__,
             newline.dwDTERate, newline.bCharFormat, newline.bParityType,
             newline.bDataBits);
  }

  if (C_CRTSCTS(tty)) {
    if (ch9344->uartmode[portnum] == M_IO) {
      ret = -EINVAL;
      goto out;
    }
    ret = ch9344_set_uartmode(ch9344, portnum, portnum, M_HF);
    if (!ret) {
      ch9344->uartmode[portnum] = M_HF;
    }
  } else {
    if (ch9344->uartmode[portnum] == M_HF) {
      ret = ch9344_set_uartmode(ch9344, portnum, portnum, M_NOR);
...
static void ch9344_read_buffers_free(struct ch9344 *ch9344) {
  ...
  for (i = 0; i < ch9344->rx_buflimit; i++)
    usb_free_coherent(usb_dev, ch9344->readsize, ch9344->read_buffers[i].base,
...
static int ch9344_probe(struct usb_interface *intf,
  ...
  dev_dbg(&intf->dev,
          "epcmdread: %d, epcmdwrite: %d, epread: %d, epwrite: %d\n",
  ...
  for (i = 0; i < num_rx_buf; i++) {
    ...
    usb_fill_bulk_urb(urb, ch9344->dev, ch9344->rx_endpoint, rb->base,
                      ch9344->readsize, ch9344_read_bulk_callback, rb);
  ...
  for (i = 0; i < CH9344_NW; i++) {
    ...
    usb_fill_bulk_urb(snd->urb, usb_dev,
                      usb_sndbulkpipe(usb_dev, epwrite->bEndpointAddress), NULL,
  ...
  for (i = 0; i < portnum; i++) {
    tty_dev =
        tty_port_register_device(&ch9344->ttyport[i].port, ch9344_tty_driver,
...
static int __init ch9344_init(void) {
  ...
  ch9344_tty_driver->init_termios.c_cflag =
